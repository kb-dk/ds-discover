...
After a fresh checkout or after the `ds-discover-openapi_v1.yaml` specification has changed, the `api` and the `model` files 
...
where "ds-discover" is your artifactID from above.

The Swagger UI is available at <http://localhost:9074/ds-discover/api/>, providing access to the `v1` version of the GUI. 
...
During development the configurations are located in the `conf`-folder as `conf/ds-discover-behaviour.yaml`,  
`conf/ds-discover-environment.yaml` and `conf/ds-discover-local.yaml`. 
When the project is started using Jetty, these are the configurations that are used.

Access to the configuration is through the static class at `src/main/java/dk.kb.discover/config/ServiceConfig.java`.

**Note**: The environment configuration typically contains sensitive information. Do not put it in open code
repositories. To guard against this, `conf/ds-discover-environment.yaml` is added to `.gitignore`. 

## Jetty

Jetty is a servlet container (like Tomcat) that is often used for testing during development.

This project can be started with `mvn jetty:run`, which will expose a webserver with the implemented service at port 9074.
...
[http://localhost:9074/<application-ID>/](http://localhost:9074/<application-ID>/).

While it is possible to use [JSP](https://en.wikipedia.org/wiki/Jakarta_Server_Pages), as the sample 
[index.jsp](ds-discover-web/src/main/webapp/index.jsp) shows, this is considered legacy technology.
...
Everything is defined centrally in the file [src/main/openapi/ds-discover-openapi_v1.yaml](ds-discover-web/src/main/openapi/ds-discover-ds-discover-openapi_v1.yaml).
IntelliJ IDEA has a plugin for editing OpenAPI files that provides a semi-live preview of the generated GUI and
the online [Swagger Editor](https://editor.swagger.io/) can be used by copy-pasting the content of `ds-discover-openapi_v1.yaml`.
...
A common pattern during initial definition of the `ds-discover-openapi_v1.yaml` is to delay the implementation phase and recreate 
the skeleton implementation files on each build. This can be done by setting `generateOperationBody` in the `pom.xml` 
to `true`.

**Tip:** If the `ds-discover-openapi_v1.yaml` is changed a lot during later development of the application, it might be better to have 
`<generateOperationBody>true</generateOperationBody>` in `pom.xml` and add the implementation code to manually created
classed (initially copied from the OpenAPI-generated skeleton impl classes). When changes to `ds-discover-openapi_v1.yaml` results in
...
See how we map exceptions to responsecodes in [ServiceExceptionMapper](ds-discover-web/src/main/java/dk/kb/webservice/ServiceExceptionMapper.java) 

See [ServiceException](ds-discover-web/src/main/java/dk/kb/webservice/exception/ServiceException.java) and its specializations for samples.

### Mustache templates

The templates in [src/main/templates/](ds-discover-web/src/main/templates/) overrides the default Swagger templates.
...
should
  ...
  * Create a new OpenAPI YAML, e.g. `openapi_v2.yaml` (preferably by copying `ds-discover-openapi_v1.yaml` and adjusting) and place
    it alongside `ds-discover-openapi_v1.yaml`
    * Edit the old `ds-discover-openapi_v1.yaml` and add a note that a new version is available
  ...
  * Add the path to the new OpenAPI YAML to `urls` in `webapp/api/index.html`. The first entry in the array is the
    default when visiting the main API-page at <http://localhost:9074/ds-discover/api/>  
...
```
[Unreleased](https://github.com/kb-dk/ds-discover-template/compare/v1.0.0...HEAD)
[1.1.0](https://github.com/kb-dk/ds-discover-template/compare/v1.0.0...v1.1.0)
[1.0.0](https://github.com/kb-dk/ds-discover-template/releases/tag/v1.0.0)
```

For BitBucket (KB's internal git), the syntax is
```
[Unreleased](https://sbprojects.statsbiblioteket.dk/stash/projects/ARK/repos/ds-discover-template/compare/commits?targetBranch=refs%2Ftags%2Fv1.1.0&sourceBranch=refs%2Fheads%2Fmaster)
[1.1.0](https://sbprojects.statsbiblioteket.dk/stash/projects/ARK/repos/ds-discover-template/compare/commits?targetBranch=refs%2Ftags%2Fv1.0.0&sourceBranch=refs%2Ftags%2Fv1.1.0)
[1.0.0](https://sbprojects.statsbiblioteket.dk/stash/projects/ARK/repos/ds-discover-template/commits?until=refs%2Ftags%2Fv1.0.0)
```
Note that both the `ARK`-part and the repo-id `ds-discover-template` is project-specific.


## Release procedure

1. Review that the `version` in `pom.xml` is fitting. `ds-discover` uses
...
which boils down to
   ...
   * Run `git push`   
   ...
   `mvn release:rollback ; git tag -d ds-discover-1.4.2 ; git push --delete origin ds-discover-1.4.2`

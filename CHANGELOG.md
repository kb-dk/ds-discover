...
- ServiceConfig singleton as in other ds-modules. SolrManager will no longer automatic register changes in
  configuration. If solr collection name or url is changed, ds-discover most be restarted
...
- Bumped SwaggerUI dependency to v5.18.2
- Bumped multiple OpenAPI dependency versions
- Added injection of Oauth token on all service methods when using DsDiscoverClient. But no methods for new are exposed
  in the client.
- Bumped kb-util to v1.6.9 for service2service oauth support.
- Removed auto generated DsDiscoverClient class that was a blocker for better exception handling. All DsDiscoverClient
...
- The SolrService class now delivers the correct error message and status code from solr, when the backing solr throws
  an HTTP code outside the range 200-299.
...
- Yaml configuration updated with list of additional allowed Solr query parameters (solr.extraAllowedParameters).
  SolrShield will also allow these parameters can be returned from Solr.
...
- Changed default param values for /select endpoint to match default solr
  configuration. [DRA-564](https://kb-dk.atlassian.net/browse/DRA-564)
...
- SolrShield implemented for basic search + facet. Currently set to log the result instead of potentially rejecting
  calls to select/

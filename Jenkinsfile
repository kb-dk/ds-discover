            ...
            steps {
                script {
                    if ( env.ORIGINAL_BRANCH ==~ "PR-[0-9]+" ) {
                        def empty_if_no_branch = sh(script: "git ls-remote --heads https://github.com/kb-dk/${env.BUILD_TO_TRIGGER}.git | grep 'refs/heads/${env.SOURCE_BRANCH}' || empty", returnStdout:true).trim()
